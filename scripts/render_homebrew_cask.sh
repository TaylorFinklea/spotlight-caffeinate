...
usage() {
  cat <<'EOF'
Render the Homebrew cask for the signed app release artifact.

Usage:
  ./scripts/render_homebrew_cask.sh [--version <version>] [--sha256 <sha256>]

Defaults:
  --version uses MARKETING_VERSION from project.yml
  --sha256 uses build/SpotlightCaffeinate.zip when present
EOF
}
...
if [[ -z "$sha256" ]]; then
  if [[ ! -f "$archive_path" ]]; then
    echo "Missing $archive_path. Build the signed release or pass --sha256 explicitly." >&2
    exit 1
  fi
